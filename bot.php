...
$MadelineProto = new API('ShahreSource.madeline', $settings);
##------------------Variables-----------------##


##---------------Event Handler----------------##
class ShahreSource extends EventHandler
{
    /**
    ...
    public function onUpdateNewMessage(array $update): \Generator
    {
        ...
        if (isset($update['message']['message'])) {
            ...
            $ShahreSource = '<a href="mention:@ShahreSource">ShahreSource</a>';
            $ShahreSource = '<a href="mention:@ShahreSource">ShahreSource</a>';
            $me = $this->getSelf();
            $me_id = $me['id'];
            $sudo = $GLOBALS['dev'];
            ...
            try {
                ...
                if ($userID === $sudo or isset($data['data']['Admins'][$userID])) {
                    ...
                    if (in_array($msg, ['state', 'vaziat', 'آنلاین✅', 'ربات', 'آنلاین', 'پینگ'])) {
                        $text = "ربات $state است." . PHP_EOL . "$ShahreSource";
                        yield $this->messages->sendMessage(['peer' => $chatID, 'parse_mode' => 'html', 'message' => $text, 'reply_to_msg_id' => $msg_id]);
                    }
                    ##==========================SETTINGS==========================##
                    if ($state === "آنلاین✅") {
                                    ...
                                    foreach ($links as $link) {
                                        ...
                                        if (isset($check['title']) and $check['_'] !== "chatInviteAlready") {
                                            ...
                                            if (isset($joined['chats'])) {
                                                yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "تبچی ShahreSource در یک لینک عضو شد♻️ \nCreator: $ShahreSource ", 'parse_mode' => 'html']);
                                                yield $this->sleep(3);
                                            } else {
                        ...
                        if ($JoinSave === "آنلاین✅") {
                            if (isset($data["data"]["links"]) and count($data["data"]["links"]) > 0) {
                                ...
                                yield $this->messages->importChatInvite([
                                    'hash' => $link]);
                                yield $this->messages->sendMessage(['peer' => $me_id, 'message' => "♻️تبچی ShahreSource در یک لینک ذخیره شده عضو شد", 'parse_mode' => 'html']);
                    ...
                    if ($state === "آنلاین✅") {
                        ...
                        if (preg_match("/^[#!\/]?(join) (.*)$/i", $msg)) {
                            if (preg_match("/^[#!\/]?(join) (on|off)$/i", $msg, $mch)) {
                                if (preg_match("/on/i", $mch[2])) {
                                    ...
                                    savedata($data);
                                    yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️جوین تبچی ShahreSource روشن شد", 'parse_mode' => "html"]);
                                    sleep(2);
                                } else {
                                    $data["data"]["Join"] = "خاموش❌";
                                    savedata($data);
                                    yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️جوین تبچی ShahreSource خاموش شد!", 'parse_mode' => "html"]);
                                    sleep(2);
                                }
                            } else if (preg_match("/^[#!\/]?(join) @(.*)$/i", $msg, $mch)) {
                                $a = yield $this->channels->joinChannel(['channel' => "@$mch[2]",]);
                                if (isset($a)) {
                                    yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️تبچی ShahreSource در @$mch[2] عضو شد!", 'parse_mode' => "html"]);
                                }
                            }
                        }
                        if (preg_match("/^[#!\/]?(leave)/i", $msg)) {
                            if (preg_match("/^[#!\/]?(leave)$/i", $msg)) {
                                ...
                                if (isset($a)) {
                                    yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️تبچی ShahreSource از @$mch[2] خارج شد!", 'parse_mode' => "html"]);
                                }
                            }
                        }
                        if (preg_match("/^[#!\/]?(Save) (on|off)$/i", $msg, $mch)) {
                            if (preg_match("/on/i", $mch[2])) {
                                ...
                                savedata($data);
                                yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️ذخیره لینک های جوین نشده تبچی ShahreSource روشن شد", 'parse_mode' => "html"]);
                                sleep(2);
                            } else {
                                $data["data"]["Save"] = "خاموش❌";
                                savedata($data);
                                yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️ذخیره لینک های جوین نشده تبچی ShahreSource خاموش شد!", 'parse_mode' => "html"]);
                                sleep(2);
                            }
                        }
                        if (preg_match("/^[#!\/]?(joinsave) (on|off)$/i", $msg, $mch)) {
                            if (preg_match("/on/i", $mch[2])) {
                                ...
                                savedata($data);
                                yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️جوین لینک های ذخیره شده تبچی ShahreSource روشن شد", 'parse_mode' => "html"]);
                                sleep(2);
                            } else {
                                $data["data"]["JoinSave"] = "خاموش❌";
                                savedata($data);
                                yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️جوین لینک های ذخیره شده تبچی ShahreSource خاموش شد!", 'parse_mode' => "html"]);
                                sleep(2);
                            }
                        }
                        if (preg_match("/^[#!\/]?(read) (on|off)$/i", $msg, $mch)) {
                            if (preg_match("/on/i", $mch[2])) {
                                ...
                                savedata($data);
                                yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️تیک خواندن تبچی ShahreSource روشن شد", 'parse_mode' => "html"]);
                                sleep(2);
                            } else {
                                $data["data"]["Read"] = "خاموش❌";
                                savedata($data);
                                yield $this->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "♻️تیک خواندن تبچی ShahreSource خاموش شد!", 'parse_mode' => "html"]);
                                sleep(2);
                            }
                        }
                        if ($Read === "آنلاین✅") {
           ...
           ", 'parse_mode' => 'markdown']);
            ...
            ", 'parse_mode' => 'html']);
                        ...
                        if (preg_match("/^[#!\/]?(profile) /i", $msg)) {
                            ...
                            $id1 = (isset($mch[2]) && $mch[2] !== "" && $mch[2] !== " ") ? $mch[2] : "ShahreSource";
                            $id2 = (isset($mch[3])) ? $mch[3] : "";
                            $id3 = (isset($mch[4]) && $mch[4] !== "" && $mch[4] !== " ") ? $mch[4] : "@ShahreSource | PHP Developer";
...
$MadelineProto->startAndLoop(ShahreSource::class);
